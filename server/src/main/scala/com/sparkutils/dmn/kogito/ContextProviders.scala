...
import com.sparkutils.dmn.kogito.{types => kogitotypes}
...
case class KogitoJSONContextProvider(contextPath: dmn.DMNContextPath, stillSetWhenNull: Boolean, child: Expression,
                                     providedType: Option[DataType]) extends dmnimpl.StringWithIOProcessorContextProvider[java.util.Map[String, Object]] {
 ...
 * @param config
 */
case class ContextProviderProxy(contextPath:  dmn.DMNContextPath, stillSetWhenNull: Boolean,
                                child: Expression, config: Map[String, String], providedType: Option[DataType]) extends dmn.UnaryDMNContextProvider[Any] {
...
object ContextProviders {
  private[kogito] def contextProviderFromDDL(stillSetWhenNull: Boolean, path: KogitoDMNContextPath, expr: Expression, config: Map[String, String], dataType: DataType) = 
    dataType match {
      case StringType => dmnimpl.StringContextProvider(path, stillSetWhenNull, expr, providedType = Some(dataType))
      case IntegerType => dmnimpl.SimpleContextProvider[Integer](path, stillSetWhenNull, expr, providedType = Some(dataType))
      case LongType => dmnimpl.SimpleContextProvider[Long](path, stillSetWhenNull, expr, providedType = Some(dataType))
      case BooleanType => dmnimpl.SimpleContextProvider[Boolean](path, stillSetWhenNull, expr, providedType = Some(dataType))
      case DoubleType => dmnimpl.SimpleContextProvider[Double](path, stillSetWhenNull, expr, providedType = Some(dataType))
      case FloatType => dmnimpl.SimpleContextProvider[Float](path, stillSetWhenNull, expr, providedType = Some(dataType))
      case BinaryType => dmnimpl.SimpleContextProvider[Array[Byte]](path, stillSetWhenNull, expr, providedType = Some(dataType))
      case ByteType => dmnimpl.SimpleContextProvider[Byte](path, stillSetWhenNull, expr, providedType = Some(dataType))
      case ShortType => dmnimpl.SimpleContextProvider[Short](path, stillSetWhenNull, expr, providedType = Some(dataType))
      case DateType => dmnimpl.SimpleContextProvider[LocalDate](path, stillSetWhenNull, expr,
      ...
      case TimestampType | TimestampNTZType => dmnimpl.SimpleContextProvider[LocalDateTime](path, stillSetWhenNull, expr,
      ...
      case _: DecimalType => dmnimpl.SimpleContextProvider[java.math.BigDecimal](path, stillSetWhenNull, expr,
      ...
      case structType: StructType => kogitotypes.ContextInterfaces.structProvider(structType, path, expr, stillSetWhenNull, config)
      case mapType: MapType => kogitotypes.ContextInterfaces.mapProvider(mapType, path, expr, stillSetWhenNull, config)
      case arrayType: ArrayType => kogitotypes.ContextInterfaces.arrayProvider(arrayType, path, expr, stillSetWhenNull, config)
      // calendar interval?
      case t => throw new dmn.DMNException(s"Provider type $t is not supported")
    }
